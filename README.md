...
4. **Build with pygbag** – compiles the Pygame app to WebAssembly via `python -m pygbag --build --ume_block 0 --template web/template.tmpl pong`. The custom template (`web/template.tmpl`) integrates the startup overlay (title, controls, Play button) directly into the same page as the game canvas, so the WASM runtime begins loading the moment the page opens. Clicking **▶ PLAY** simply fades out the overlay to reveal the already-loading game. The output is written to `pong/build/web/`.
